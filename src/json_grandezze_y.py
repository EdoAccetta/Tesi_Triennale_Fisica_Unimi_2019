    ...
    for file in list_files:
        ...
        with open(path_acc_data_json + file, "r") as file_acc:
            ...
            for x in range(0, json_object["pre"]):
                ...
                for j in range (25, 9, -1):
                    if j % 2 == 0:
                        continue
                    if index_minimo - latest_min_ind > 30:
                        index_minimo = _find_first_minimo(gfys, j)
                    else:
                        break
            ...
            for x in range(0, json_object["into"]):
                index_last_minimo += find_first_minimo(gfys[index_last_minimo:])
                for j in range (25, 9, -1):
                    if j%2 == 0:
                        continue
                    if index_last_minimo - index_precedente > 30:
                        index_last_minimo = index_precedente + _find_first_minimo(gfys[index_precedente:], j)
                    else:
                        break
            ...
            for x in range(0, len(minimi_array_index) - 2):
                ...
                # for j in range(minimi_array_index[x], minimi_array_index[x+1]):
                    # if(massimo < gfzs[j]):
                        # massimo = gfzs[j]
                        # index_massimo = j
                array_in_campione = gfys[minimi_array_index[x]:minimi_array_index[x+1]]
                ...
                print("len(gfzs): {}".format(len(gfys)))
